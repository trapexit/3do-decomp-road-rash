                                       ...
                                       DSRequestMsgPtr request)
{
  ...
  result = find_data_stream_subscriber(
    stream, request->msg.setChannel.streamType,
    &subscriber);
  ...
  message->whatToDo = kStreamOpSetChan;
  message->privatePtr = request;
  message->msg.channel.number =
    request->msg.setChannel.channelNumber;
  message->msg.channel.status =
    request->msg.setChannel.channelStatus;
